<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="xiangmuYuyueYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.xiangmuYuyueYesnoTypes" placeholder="请选择报名状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.xiangmuYuyueYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="xiangmuYuyueYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="xiangmuYuyueYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '报名编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xiangmuYuyueUuidNumber" placeholder="报名编号" clearable></el-input>
                     </el-form-item>
                                                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '报名状态' : ''">
                        <el-select v-model="searchForm.xiangmuYuyueYesnoTypes" placeholder="请选择报名状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xiangmuYuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '保险类型' : ''">
                        <el-select v-model="searchForm.xiangmuTypes" placeholder="请选择保险类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xiangmuYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yanglaobapxianguanlixit/upload/xiangmuYuyueMuBan.xls"
                        >批量导入保险参保数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiangmuYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xiangmuYuyueUploadSuccess"
                                :on-error="xiangmuYuyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xiangmuYuyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入保险参保数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiangmuYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xiangmuYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuNewMoney"
                                      header-align="center"
                                      label="金额">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuNewMoney}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuYuyueUuidNumber"
                                   header-align="center"
                                   label="报名编号">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuYuyueUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuYuyueText"
                                      header-align="center"
                                      label="报名理由">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xiangmuYuyueText != null &&scope.row.xiangmuYuyueText.length>10">
                                {{scope.row.xiangmuYuyueText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xiangmuYuyueText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuYuyueCunzhen"
                                   header-align="center"
                                   label="所属村镇">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuYuyueCunzhen}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuYuyueDizhi"
                                   header-align="center"
                                   label="家庭地址">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuYuyueDizhi}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuYuyueYesnoTypes"
                                      header-align="center"
                                      label="报名状态">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuYuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuYuyueYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xiangmuYuyueYesnoText != null &&scope.row.xiangmuYuyueYesnoText.length>10">
                                {{scope.row.xiangmuYuyueYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xiangmuYuyueYesnoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuYuyueShenheTime"
                                   header-align="center"
                                   label="审核时间">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuYuyueShenheTime}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuYuyueTime"
                                   header-align="center"
                                   label="参保日期">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuYuyueTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xiangmuYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xiangmuYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xiangmuYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('xiangmuYuyue','审核') && scope.row.xiangmuYuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              xiangmuYuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                xiangmuYuyueUuidNumber : null,
                xiangmuId : null,
                yonghuId : null,
                yuangongId : null,
                xiangmuYuyueText : null,
                insertTime : null,
                xiangmuYuyueCunzhen : null,
                xiangmuYuyueDizhi : null,
                xiangmuYuyueYesnoTypes : null,
                xiangmuYuyueYesnoText : null,
                xiangmuYuyueShenheTime : null,
                xiangmuYuyueTime : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            xiangmuYuyueYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '员工邮箱': 'yuangongEmail',
                //本表字段
                     '报名编号': "xiangmuYuyueUuidNumber",
                     '报名理由': "xiangmuYuyueText",
                     '项目报名时间': "insertTime",
                     '所属村镇': "xiangmuYuyueCunzhen",
                     '家庭地址': "xiangmuYuyueDizhi",
                     '报名状态': "xiangmuYuyueYesnoValue",
                     '审核回复': "xiangmuYuyueYesnoText",
                     '审核时间': "xiangmuYuyueShenheTime",
                     '参保日期': "xiangmuYuyueTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xiangmu_yuyue',//当前表表名,
                        // sumColum : 'xiangmu_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xiangmu_yuyue_name',//分组字符串字段
                        types : 'xiangmu_yuyue_yesno_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "xiangmu_yuyue_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.xiangmuYuyueUuidNumber!= '' && this.searchForm.xiangmuYuyueUuidNumber!= undefined) {
                    params['xiangmuYuyueUuidNumber'] = '%' + this.searchForm.xiangmuYuyueUuidNumber + '%'
                }
                                                                 
                if (this.searchForm.xiangmuYuyueYesnoTypes!= '' && this.searchForm.xiangmuYuyueYesnoTypes!= undefined) {
                    params['xiangmuYuyueYesnoTypes'] = this.searchForm.xiangmuYuyueYesnoTypes
                }
                                
                params['xiangmuYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xiangmuYuyue/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xiangmu_yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xiangmuYuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            xiangmuYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xiangmuYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            xiangmuYuyueUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.xiangmuYuyueYesnoText = null;
                _this.form.xiangmuYuyueYesnoTypes = "请选择报名状态";
                _this.xiangmuYuyueYesnoTypesVisible = true;
            },

            xiangmuYuyueYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.xiangmuYuyueYesnoTypes == "请选择报名状态"){
                    _this.$message.error("请选择报名状态");return false;
                }
                if(_this.form.xiangmuYuyueYesnoText == null || _this.form.xiangmuYuyueYesnoText == ""){
                    _this.$message.error("请输入审核回复");return false;
                }
                this.$http({
                    url:`xiangmuYuyue/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.xiangmuYuyueYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "xiangmuYuyue/update",
                        method: "post",
                        data: {
                            id:id,
//                            xiangmuYuyueTypes:xiangmuYuyueTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
